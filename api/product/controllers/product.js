...
const postOrder = async ctx => {
    ...
    try {
        ...
        if (product.quantity < product_quantity)
            throw Error(PRODUCT_QUANTITY_EXCEED);
        if (userPoint < product_quantity * product.price)
            throw Error(MONEY_NOT_ENOUGH);
        ...
        if (productTotalPrice <= userPoint) {
            ...
            const userLastArrayLength =
                (await orderUser.orderuser_id.length) - 1;
            const orderUser_id = await orderUser.orderuser_id[
                userLastArrayLength
            ].id;
...
const getOrderDetail = async ctx => {
    ...
    const orderType = '주문';
    const refund = '환불 미정';
    const userLogs = await strapi
        .query('orderlog')
        .find({ user_id: userId, type: orderType, refund: refund });
...
const patchRefund = async ctx => {
    ...
    const orderCode = await strapi.query('orderlog').findOne({
        user_id: userId,
        order_code: orderCodeParam,
        refund: '환불 미정',
    });
    const userPoint = await findUserPoint(userId);
    const orderType = '환불';
    ...
    try {
        ...
        const product = await strapi
            .query('product')
            .findOne({ product_code: orderCode.product_code });
        ...
        const productTotalQuantity =
            orderCode.product_quantity + product.quantity;
...
const getMyLog = async ctx => {
    const userId = ctx.request.user.id;
    const orderLogData = await strapi
        .query('orderlog')
        .find({ user_id: userId });
